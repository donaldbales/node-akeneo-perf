...
import { AttributeMetric } from '../interfaces/AttributeMetric';
...
export function createMetric(logger: Logger, attributeMap: any, property: string) {
  const methodName: string = 'createMetric';
  logger.debug({ moduleName, methodName }, `Starting...`);

  const available_locales: any[] = attributeMap[property]['available_locales'] ? attributeMap[property]['available_locales'] : ['en_US'];
  const code: string = akeneo.deQuote(property) === property ? akeneo.attributeCode(property) : akeneo.attributeCode(akeneo.deQuote(property));
  const group: string = attributeMap[property]['group'] ? attributeMap[property]['group'] : defaultGroup;
  const label: string = akeneo.deQuote(property) === property ? akeneo.attributeLabel(property) : akeneo.deQuote(property);
  const localizable: boolean = attributeMap[property]['localizable'] ? true : false;
  const metric_family: string = attributeMap[property]['metric_family'] ? attributeMap[property]['metric_family'] : 'Length';
  const default_metric_unit: string = attributeMap[property]['default_metric_unit'] ? attributeMap[property]['default_metric_unit'] : 'INCH';
  const results: any = {
    attribute: {}
  };
  const scopable: boolean = attributeMap[property]['scopable'] ? true : false;
  ...
  const attribute: AttributeMetric = {
    code,
...
export function attributes(logger: Logger, attributeMap: any) {
  ...
  for (const property in attributeMap) {
    ...
    if (attributeMap.hasOwnProperty(property)) {
      ...
      if (attributeMap[property].akeneo_type === akeneo.PIM_CATALOG_METRIC) {
        const result: any = createMetric(logger, attributeMap, property);
        results.attributes.push(result.attribute);
      } else
